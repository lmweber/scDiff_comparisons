...
# note: including random effects for 'patient_id' and 'sample_id'
contrast_vec <- c(0, 1)
...
runtime_tests <- system.time({
  ...
  formula <- createFormula(metadata(d_se)$experiment_info, 
                           cols_fixed = "group_id", cols_random = c("patient_id", "sample_id"))
  formula
...
#out_diffcyt_DA_GLMM_main
#runtime_diffcyt_DA_GLMM_main
#out_clusters_diffcyt_DA_GLMM_main
#out_objects_diffcyt_DA_GLMM_main
