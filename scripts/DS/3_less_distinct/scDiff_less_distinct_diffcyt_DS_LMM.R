...
# names of replicates
rep_names <- names(data_less_distinct)

# lists to store objects
out_diffcyt_DS_LMM_less_distinct  <- 
  out_clusters_diffcyt_DS_LMM_less_distinct <- 
  out_objects_diffcyt_DS_LMM_less_distinct <- 
  runtime_diffcyt_DS_LMM_less_distinct <- vector("list", length(rep_names))
names(out_diffcyt_DS_LMM_less_distinct) <- 
  names(out_clusters_diffcyt_DS_LMM_less_distinct) <- 
  names(out_objects_diffcyt_DS_LMM_less_distinct) <- 
  names(runtime_diffcyt_DS_LMM_less_distinct) <- rep_names
...
for (s in 1:length(rep_names)) {
  ...
  out_objects_diffcyt_DS_LMM_less_distinct[[s]] <- list(
    d_se = d_se, 
    d_counts = d_counts, 
    d_medians = d_medians, 
  ...
  runtime_diffcyt_DS_LMM_less_distinct[[s]] <- runtime_total
  ...
  out_clusters_diffcyt_DS_LMM_less_distinct[[s]] <- res_clusters
  ...
  # store results
  out_diffcyt_DS_LMM_less_distinct[[s]] <- res
...
#out_diffcyt_DS_LMM_less_distinct
#runtime_diffcyt_DS_LMM_less_distinct
#out_clusters_diffcyt_DS_LMM_less_distinct
#out_objects_diffcyt_DS_LMM_less_distinct
